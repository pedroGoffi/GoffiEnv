# NOTE: stole for TSODING because i am too lazy to build my own
...
def run_test_for_file(file_path: str, stats: RunStats = RunStats()):
    ...
    assert file_path.endswith(GFL_EXT)

    print('[INFO] Testing %s' % file_path)

    tc_path = file_path[:-len(GFL_EXT)] + ".txt"
    ...
    if tc is not None:
        ...
        com = cmd_run_echoed([EXECUTABLE_NAME, file_path])
        if com.returncode != 0:
            error = True
            stats.failed += 1
        stats.ignored += 1
...
def run_test_for_folder(folder: str):
    ...
    for entry in os.scandir(folder):
        if entry.is_file() and entry.path.endswith(GFL_EXT):
